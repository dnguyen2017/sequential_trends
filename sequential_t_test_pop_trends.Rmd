...
ntsim <- 1000
sim_dat_ttest <- lapply(1:1000, function(x) rnorm(n = tmax, mean = 0, sd = 1))
...
repeated_t <- 
  rep_t_df %>%
  summarize(sample_size = unique(sample_size), 
                            ecdf = ecdf(sample_size)(unique(sample_size)) * length(sample_size))
rep_t_freq <- 
  rep_t_df %>%
  group_by(sample_size) %>%
  summarise(num = n()) %>%
  # rowwise() %>%
  mutate(freq = cumsum(num)/ntsim)

repeated_t  <-
  inner_join(rep_t_df, rep_t_freq, by = "sample_size")


repeated_t %>%
  ggplot(aes(x = sample_size, y = freq)) + geom_step() +
