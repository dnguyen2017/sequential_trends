...
ntsim <- 5000
sim_dat_ttest <- lapply(1:ntsim, function(x) rnorm(n = tmax, mean = 0, sd = 1))
...
rep_t_df %>%
  ...
  geom_point(filter(rep_t_df, sample_size %in% c(2, 10, 20, 50, 100)), 
             mapping = aes(x = sample_size, y = cum_H1)) +
  geom_text(filter(rep_t_df, sample_size %in% c(2, 10, 20, 50, 100)), 
