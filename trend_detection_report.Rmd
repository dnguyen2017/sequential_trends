...
```{r}
pop_df %>%
  ggplot() + geom_line(aes(x = time, y = pop, group = simulation)) +
...
x0_ <- 1000
t_ <- 100
pop_df <- multi_sim(nsim = numsim, x0 = x0_, t = t_, trend = 0, sd = sd_)
```
...
delay_df %>%
  ...
  geom_line(data = mintime_df, aes(x = time, y = n), size = 1.5, col = "red") +
  labs(title = "False positive rates of tests",
       subtitle = "Solid = sd unknown; Dashed = sd known; red = fixed-sample test",
...
* test performance on time-series with no trend
