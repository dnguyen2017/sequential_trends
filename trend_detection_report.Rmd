...
set.seed(123)
numsim <- 100
trend_ <- -1.5 # switch to declining trend
sd_ <- 5
sd_lower <- 3
x0_ <- 1000
t_ <- 100
...
# Potential advantages of sequential vs fixed-sample size tests

1. **Efficiency**: Sequential tests often require only an average of 40-50 % of the samples needed to make a decision compared to a corresponding fixed sample test. If decisions are made on the basis of null hypothesis testing then management decisions could be made sooner using sequential tests rather than fixed sample size tests.
1. **Controlled error rates**: It is well known that the repeated application of fixed sample size hypothesis tests inflates the probability of rejecting the null when it is true (type I error). If a manager wanted to test for a trend and control their type I error, they may only test for a trend once after a pre-determined number of observations. Until that time, the manager could not draw any conclusions from their data using any formal hypothesis tests. On the other hand, sequential tests are designed to be applied after every observation until a conclusion is drawn while controlling type I error. Therefore, sequential tests are more appropriate than fixed sample size tests for continuously monitoring a population.
...
1. sequential test ($\sigma$ unknown)
    ...
    + note that the MLE estimate of $\sigma$ is uncorrected (biased low) but this bias is negligable as the number of observations increases
...
```{r example_of_test, eval = FALSE}
sprt_est_1 <- 
  pop_df %>% filter(simulation == 1) %>% 
  calc_sprt(reject_reg = c(-5, 0), sd_est = "sd_mle", sd_min = sd_lower) %>%
  rename(decision_est = decision,
         sprt_est = sprt) %>%
  select(-lik0, -lik1)
...
# sprt_est_1 %>%
# ggplot() +
#   geom_line(aes(x = time, y = sprt_est)) +
#   geom_hline(aes(yintercept = a) ) +
#   geom_hline(aes(yintercept = b) )

sprt_est_1 %>%
  filter(time <= 30) %>%
  pivot_longer(cols = c("sprt_est", "pop"),
               names_to = "metric",
               values_to = "values") %>%
  mutate(a = ifelse(metric == "sprt_est", a, NA),
         b = ifelse(metric == "sprt_est", b, NA),
         metric = ifelse(metric == "sprt_est", "Test statistic", "Population Abundance")) %>%
ggplot() +
...
sprt_known <- 
  pop_df %>% 
  calc_sprt(reject_reg = c(-5, 0), sd_est = "sd") %>%
...
The fixed-sample size test needs `r unique(samplesize_df$mintime)` samples to achieve 0.8 power at the 0.05 significance level. In comparison, the sequential tests require `r delay_df %>% filter(method == "delay_known") %>% pull(mean_delay) %>% unique() %>% ceiling()` ($\sigma$ known) and `r delay_df %>% filter(method == "delay_est") %>% pull(mean_delay) %>% unique() %>% ceiling()` ($\sigma$ unknown and $\sigma_{min} =$ `r sd_lower`). 
<!-- The reason that the sequential test where $\sigma$ is unknown detects the change faster than when $\sigma$ is known (for the parameters used here) is because the MLE estimate of $\sigma$ is uncorrected which biases it low. This makes the likelihood of the null smaller than it should be.  -->
The table belows shows the number of instances either sequential method required as many or more samples than the fixed-sample size test.
...
# Comparison of sample sizes for different population trends and variability

As a comparison to figure 1. c-d in White (2019) I ran simulations to assess how the performance of the sequential test performs relative to the fixed sample test for the same trend and sd values used by White. For these values, the average sample size needed for the sequential test to detect a trend is roughly equal to as little as half the samples needed under the fixed sample test. This implies that in terms of timeliness, there may not be an advantage to the sequential test compared to the fixed sample size test when the magnitude of a trend is large or when variability is low. However, the fixed sample size test cannot be used to repeatedly test for the evidence of a trend due to multiple testing issues as mentioned earlier whereas the sequential test can be used repeatedly as the samples accrue. 

![](sens_trend_strength_plot.png) 

![](sens_var_strength_plot.png) 
...
The Gompertz model is commonly used to model population dynamics. It is more appropriate as a null model of no trend since it perturbations away from the carrying capacity return to the carrying capacity which is a more realistic model a stable population than the random walk model.
...
<!-- $\text{log-Gompertz: } X_{t+1} = \lambda + b (X_t) + N(0, \sigma^2)\\ -->
<!-- X_t = \text{log-population size at time t } (X_1 = \log(1000)\\ -->
<!-- \lambda = \text{population growth rate per time step } (\lambda = 2.4)\\ -->
<!-- b = \text{magnitude of density dependence } (b = 0.65)\\ -->
<!-- \sigma^2 = \text{process error } (\sigma^2=0.16)$ -->

<!-- $\text{log-Gompertz: } X_{t+1} = \lambda + b (X_t) + \delta\\ -->
<!-- X_t = \text{log-abundance at time t }\\ -->
<!-- \delta \sim N(0,\sigma^2) \text{ (process noise)}\\ -->
<!-- X_0 = \log(1000)\\ -->
<!-- \lambda = 2.4 \text{ (population growth rate) }\\ -->
<!-- b = 0.65 \text{ (density dependence)}\\ -->
<!-- \sigma^2 = 0.16$ -->


```{r cache=TRUE}
nsim = 100
test <- sim_log_gomp(xinit = log(1000), lambda =  2.4, b = 0.65, phi = 0, sd_proc = 0.4, sd_obs = 0, tfinal = 100, nsim = nsim)
...
test %>%  rename(pop = log_x) %>% 
  calc_sprt(sd_est = "sd_mle", sd_min = 0.2) %>% 
  rename(decision_est = decision) %>%   
  group_by(time) %>%
  count(decision_est, name = "n_est"))

mintime_gomp <- test %>% mutate(trend = 0) %>%  rename(pop = log_x) %>% 
  seq_lm(nsim = nsim) %>% get_conclusion(sig_level = 0.05) %>%
      group_by(time) %>%
    count(type_error) %>%
    filter(type_error %in% c("significant_true","significant_false") )
...
After about 25 years the sequential tests all reject the null in favor of the hypothesis that the population is growing. I'm not sure why the sequential tests perform so badly. While I am using a one-sided alternative for the sequential tests here ($H_1: r > 0$) I don't think that would account for the poor performance. Maybe is an issue with the likelihood model? Or the decision boundaries? A potential fix for this issue would be to use a composite null hypothesis ($H_0: r \in [r_{01},r_{02}]$ where $r_{01} < r_{02}$) instead of the current point null hypothesis that the trend is 0 ($H_0: r = 0$). The bounds of the composite null can be chosen according to specific management objectives or IUCN red list criteria. 

The type 1 error of the fixed-sample test never exceeds 50 % for the sample sizes considered here. While this exceeds the nominal type I error (0.05) it is less than the type I error for the sequential test. Although, even the fixed-sample size procedure would reject the null hypothesis with probability 1 as the sample size increases because the t-test statistic is an increasing function of the sample size. Additionaly, recall that the sub-sampling procedure that ecologists use for power calculations only rejects the null at least $1 - \beta$ (80 %) of the individual regressions fit to the independentlys generated time series rejects the null hypothesis. In practice, an ecologist would only have one time series, and would only be able to fit the model and draw a conclusion once after the pre-determined number of samples have been taken if they wanted to maintain a type I error probability of $\alpha$.



```{r eval = FALSE}
# try using iid normal as data to test performance when null is true
test <- test %>% mutate(pop = rnorm(n = 10000, mean = 0, sd = 0.1))
